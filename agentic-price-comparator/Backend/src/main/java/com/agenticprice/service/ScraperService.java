...
public class ScraperService {
    ...
    public void saveResults(String rawQuery, List<PriceResult> results) {
        String normalizedName = openAIService.normalizeProductName(rawQuery);
        ...
        for (PriceResult result : results) {
            try {
                Retailer retailer = findOrCreateRetailer(result.getRetailerName());
    ...
    private Retailer findOrCreateRetailer(String name) {
        return retailerRepository.findByNameIgnoreCase(name).orElseGet(() -> {
            try {
                ...
                return retailerRepository.findByNameIgnoreCase(name)
                        .orElseThrow(() -> new RuntimeException("Failed to find or create retailer: " + name));
            }
        });
    }
